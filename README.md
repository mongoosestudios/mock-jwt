...
Provide a request body with any custom claims you would like added to the token:
```bash
...
To change the custom claims, simply send a new POST request to the setclaims endpoint.  An empty body will clear the custom claims. 


The /auth/.well-known/jwks.json endpoint will return a JSON Web Key formatted response that should be compliant with RFC7517.  Pointing your authentication validation code at this endpoint should be sufficient to allow it to validate tokens issued by go-jwt.
